	...
	run: async (client, interaction) => {
		...
		if (type === "slayer") {
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(
					"Here are the top 10 slayer levels in the server"
				)
				.addFields(
			...
			const embed = new EmbedBuilder()
				...
				.setDescription(
					"Here are the top 10 wealthiest members in the server"
				)
				.addFields(
					leaderboard.map((member, index) => ({
						name: `#${index + 1} - ${member.rsn}`,
			...
			const leaderboard = triviaLeaderboard(client, 10);

			const embed = new EmbedBuilder()
				.setTitle("Trivia Wins Leaderboard")
				.setDescription(
					"Here are the top 10 members with the most trivia wins in the server"
				)
				.addFields(
					leaderboard.map((member, index) => ({
						name: `#${index + 1} - ${member.rsn}`,
						value: `<:trivia:1225161044251508767> ${member.triviaWins}`,
					}))
				);
			return await interaction.editReply({ embeds: [embed] });
		}
...
const triviaLeaderboard = (client: ExtendedClient, limit: number) => {
	const members = client.members.getAllMembers();
	const sorted = members.sort((a, b) => b.triviaWins - a.triviaWins);
	sorted.slice(0, limit);
	return sorted;
};
